...
c
        ...
        iptr          = isptrs(1,ichild)
        ichildptrs(i) = iptr
c
        krankout      = w(iptr+1)
        krankin       = w(iptr+2)
        iiout         = w(iptr+3)+iptr-1
        iiin          = w(iptr+4)+iptr-1
c
 ...
 2200 continue
c
        call prinf("ichildptrs = *",ichildptrs,nchildren)
...
c
     ...
     -    ineartars,nnearsrcs,inearsrcs,naux,evalaux,nin,evalin,
     -    nout,evalout,krankout,iout,krankin,iin,w,w(ifree),lfree,
     -    lkeep)
        if (ier .ne. 0) return
        isptrs(1,iregion) = ifree
        isptrs(2,iregion) = lkeep
...
c
        ...
        subroutine scattering_create(ier,disc,eps,zk,eval,nidxs,idxs,
     -    nin,evalin,nout,evalout,naux,evalaux,nneartars,ineartars,
...
c
        ...
        lkeep = 0
c
        nin0  = nin  + nnearsrcs + naux
        nout0 = nout + nneartars + naux
c
c       Allocate memory for the routine.
c        
        iaint     = 1
        laint     = 2*nidxs*(nin0)
c
...
c
        iatars    = irint+lrint
        latars    = 2*nneartars*nidxs
c
        iasrcs    = iatars+latars
        lasrcs    = 2*nnearsrcs*nidxs
c
        iaout     = iasrcs+lasrcs
        laout     = 2*nidxs*(nout0)
c
...
c
c       Call an auxilliary subroutine to shape arrays.
c
        call scattering_create0(ier,eps,disc,zk,nidxs,idxs,eval,naux,
     -   evalaux,nin0,nin,evalin,nnearsrcs,inearsrcs,nout0,nout,
     -   evalout,nneartars,ineartars,w(it),w(iaint),w(irint),w(iaout),
     -   w(irout),w(iicolsout),krankout,iout,w(iicolsin),krankin,iin,
     -   w(iw2),lw2,w(iasrcs),w(iatars))
        if (ier .ne. 0) return
c
c       Form the scattering matrix structure.
c
        ismatr0  = 7
        lsmatr0  = 2*krankin*krankout
c
        iiout0   = ismatr0+lsmatr0
        liout0   = krankout
c
        iiin0    = iiout0+liout0
        liin0    = krankin
...
c
        do 1000 i=1,lsmatr0
        w(ismatr0+i-1) = w(iw2+i-1)
 1000 continue
c
c       Copy over the lists of chosen source and target nodes.
c
        call scattering_imove(krankout,iout,w(iiout0))
        call scattering_imove(krankin,iin,w(iiin0))
c
        w(1)     = lkeep
        w(2)     = krankout
        w(3)     = krankin
...
c
        ...
        subroutine scattering_create0(ier,eps,disc,zk,nidxs,idxs,eval,
     -   naux,evalaux,nin0,nin,evalin,nnearsrcs,inearsrcs,nout0,nout,
     -   evalout,nneartars,ineartars,t,aint,rint,aout,rout,icolsout,
     -   krankout,iout,icolsin,krankin,iin,w,lw,asrcs,atars)
        implicit double precision (a-h,o-z)
        double complex zk,val
c
        double complex t(nidxs,nidxs)
        double complex aint(nin0,nidxs),rint(nidxs,nidxs)
        double complex aout(nout0,nidxs),rout(nidxs,nidxs)
        double complex asrcs(nidxs,nnearsrcs),atars(nneartars,nidxs)
c
        dimension inearsrcs(1),ineartars(1)
c
        dimension eval(13,1),evalaux(13,1),evalin(13,1),evalout(13,1)
        dimension w(1),idxs(1),disc(1),icolsout(1),iout(1),iin(1)
        dimension icolsin(1)
c
c       Form and invert the restricted operator T.
c
        ikernel = 3
     ...
     -    nidxs,idxs,t,w,lw)
        ...
        t(i,i) = t(i,i) + 0.5d0 
 1000 continue
        call lapack_invert(ier,nidxs,t,w,lw)
        if (ier .ne. 0) return
c
c       Form the matrix Aout(nidxs,nout0) and factor it as 
c
c           Aout(nout0,nidxs) = Bout(nout0,krankout) * Rout(krankout,nidxs)
c
        ikernel = 3
        call disc_eval(ier,disc,ikernel,zk,nneartars,ineartars,
     -    nidxs,idxs,atars,w,lw)
        if (ier .ne. 0) return
c
        do 2000 j=1,nidxs
        do 2100 i=1,nout
        call ksingle(evalout(1,i),eval(1,idxs(j)),zk,val)
        aout(i,j) = val
 2100 continue
        do 2200 i=1,naux
        call ksingleprime(evalaux(1,i),eval(1,idxs(j)),zk,val)
        aout(nout+i,j) = val
 2200 continue
 ...
 2000 continue
c
        if (lw .le. 2*(3*nidxs+1)*nout+1) then
        ier = 4
        return
        endif
c
        call factor_right(eps,nout0,nidxs,aout,krankout,icolsout,rout,w)
        do 2500 i=1,krankout
        iout(i) = idxs(icolsout(i))
 2500 continue
c
...
c
        ikernel = 3
        call disc_eval(ier,disc,ikernel,zk,nidxs,idxs,nnearsrcs,
     -    inearsrcs,asrcs,w,lw)
        if (ier .ne. 0 ) return
c
        do 3000 j=1,nidxs
        do 3100 i=1,nin
        call ksingleprime(eval(1,idxs(j)),evalin(1,i),zk,val)
        aint(i,j) = val
 3100 continue
        do 3200 i=1,naux
        call ksingleprime(eval(1,idxs(j)),evalaux(1,i),zk,val)
        aint(nin+i,j) = val
 3200 continue
 ...
 3000 continue
c
        if (lw .le. 2*(3*nidxs+1)*nin+11) then
        ier = 4
        return
        endif
c
        call factor_right(eps,nin0,nidxs,aint,krankin,icolsin,rint,w)
c
        do 3600 i=1,krankin
        iin(i) = idxs(icolsin(i))
 3600 continue
c
c       Form the matrix Rout(krankout,nidxs) * T^{-1} * Rint(nidxs,krankin)
c
        ib = 1
        lb = 2*krankout*krankin
c
        ia = ib+lb
        la = 2*krankout*nidxs
c
        call apply_left(nidxs,nidxs,krankout,icolsout,rout,t,w(ia))
        call apply_rightt(nidxs,krankout,krankin,icolsin,rint,w(ia),
     -   w(ib))
c
        call prinf("in create_smatr, krankout = *",krankout,1)
        call prinf("in create_smatr, iout = *",iout,krankout)
        call prinf("in create_smatr, krankin = *",krankin,1)
        call prinf("in create_smatr, iin = *",iin,krankin)
c
        end


c$$$        subroutine scattering_create(ier,disc,eps,zk,eval,nidxs,
c$$$     -    idxs,nin,evalin,nout,evalout,naux,evalaux,nneartars,ineartars,
...
c$$$     -    nidxs,idxs,w(ismatr),w(iw2),lw2)
c$$$c
c$$$        call scattering_addi(nidxs,w(ismatr))
c$$$c
c$$$        call lapack_invert(ier,nidxs,w(ismatr),w(iw2),lw2)
c$$$        if (ier .ne. 0) return
c$$$c
c$$$        krankin  = nidxs
c$$$        krankout = nidxs
...
c$$$        lkeep   = iiout+liout
c$$$c
c$$$        call scattering_move(krankin,iin,w(iiin))
c$$$        call scattering_move(krankout,iout,w(iiout))
c$$$c
c$$$        w(1)    = lkeep
c$$$        w(2)    = krankout
c$$$        w(3)    = krankin
...
c$$$        end
c$$$
c$$$        subroutine scattering_addi(n,a)
c$$$        implicit double precision (a-h,o-z)
