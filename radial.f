...
c
        double precision, allocatable :: xsadap(:),ysadap(:),whtsadap(:)
        double precision, allocatable :: xs(:),ys(:),whts(:)
        double precision, allocatable :: xs0(:),ys0(:),whts0(:)
        double precision, allocatable :: rad(:),errs(:),w(:)
c
        external funadap,funpoly
c
        call prini(6,13)
        pi=acos(-1.0d0)
c
...
c
        maxquad = 50 000
c
        allocate(xsadap(maxquad),ysadap(maxquad),whtsadap(maxquad))
        allocate(xs(maxquad),ys(maxquad),whts(maxquad))
        allocate(xs0(maxquad),ys0(maxquad),whts0(maxquad))
        allocate(w(1 000 000))
c
        call mach_zero(eps)
        eps = eps*100
c
c       Initialize the precomputed quadratures.
c
        norder = 8
        call radinit(ier,norder,rad,lrad,lkeep)
c
        call prinf("after radial_init, ier = *",ier,1)
        if (ier .ne. 0) stop
        call prinf("after radial_init, lkeep = *",lkeep,1)
c
c       Set the triangle for the test.
c
        x0 =  0.999900d0
        y0 =  0.000001d0
c
...
c
        ...
        verts(2,3) = 1.0d0-y0
c
        call trimap(verts,amap,ainv,det)
        call prin2("verts=*",verts,6)
        call prin2("ainv=*",ainv,6)
c
cccccccccc
c       First, test the integration of koornwinder polynomials.
cccccccccc
c
...
c
        do 0100 idx=1,npoly
c
        call cadaptri(ier,eps,verts,funpoly,ainv,idx,norder,par4,
     1    par5,par6,par7,par8,sum1,nadap)
c
        call radquad(ier,rad,verts,maxquad,nquad,xs,ys,whts)
        if (ier .ne. 0) then
        print *,"raddiag failed"
        stop
        endif
c
...
c
        ...
        y   = ys(i)
        wht = whts(i)
        call funpoly(x,y,ainv,idx,norder,par4,par5,par6,par7,par8,val)
        sum2 = sum2 + val*wht
 1100 continue
c
        errabs    = abs(sum1-sum2)
        errs(idx) = errabs 
        errmax    = max(errmax,errabs)
 0100 continue
c
        call prin2("polynomial errs=*",errs,npoly)
        call prin2("polynomial errmax = *",errmax,1)
c
cccccccccc
c       Now integrate a radially singular function.
ccccccccccc
c
        n      = 400
        call elapsed(t1)
        call radquad0(ier,verts,n,norder,maxquad,nquad0,xs0,ys0,whts0,w)
        call elapsed(t2)
        print *,"radquad0 time = ",t2-t1," secs"
c
        if (ier .ne. 0) then
        call prinf("after radquad, ier = *",ier,1)
        stop
        endif
c
        call elapsed(t1)
        ...
        wht = whts0(i)
c
        call funadap(x,y,val,par1,par2,par3,par4,par5,par6,par7,par8)
        sum1 = sum1 + val*wht
 3000 continue
c
        sum2 = 0
...
c        print *,sum2
        ...
        subroutine funpoly(s,t,amap,idx,norder,par4,par5,par6,par7,par8,
     -   val)
        implicit double precision (a-h,o-z)
        double complex val
        dimension pols(10 000),amap(2,3)
c
        x = amap(1,1)*s+amap(1,2)*t+amap(1,3)
        y = amap(2,1)*s+amap(2,2)*t+amap(2,3)
c
...
c
        ...
        subroutine funadap(x,y,val,par1,par2,par3,par4,par5,par6,par7,
     -    par8)
        implicit double precision (a-h,o-z)
        double complex val
        dimension pols(10 000),ainv(2,3)
        r = sqrt(x**2+y**2)
        t = atan2(y,x)
c
c        val = sin(t)/r**2 + 1/r +sin(7*t)+r**2+cos(7*t)*r**7
        val = 1/r +sin(7*t)+r**2+cos(7*t)*r**7
...
c
        ...
        subroutine trimap(verts,amap,ainv,det)
        implicit double precision (a-h,o-z)
        dimension verts(2,3),amap(2,3),ainv(2,3)
c
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
c       The code contained in this file generates quadrature rules for 
c       the evaluation of a class of singular integrals over more or
c       less arbitrary triangles in the plane.  More specifically,
c       the quadratures generated by this code handle integrals
c       of the form
c
c         P.V. \int  f(x,y) dA
c
c       where:
c
c       (1) f admits representation as 
c
c                                   N
c         f(rcos(t),rsin(t)) =    \sum  r^j p_j(t)                 (2)
c                                 j=-1 
c
c       with q(t) a trigonmetric polynomial of order 1 and the p_j(t)
c       trigonmetric polynomials of order 3(j+1)+2; and
c
c       (2) T is a more less arbitrary triangle containing the origin.
c
c       The quadrature rules are constructed by rewriting (1) in polar
c       coordinates as 
c
c          2\pi  R(t)
c         \int  \int  f(rcos(t), rsin(t)) r dr dt                         (3)
c           0    0
c
c       Specialized precomputed quadratures contained in the subroutine
c       pvquad (see below) are used to evaluate the inner integral in
c       (3) --- that is, the integral in the variable r.
c 
c       Depending on the user's preference, either fixed-order piecewise
c       Legendre rules or specially precomputed quadrature rules stored
c       on disk are used to evaluate the outer integral (in the variable t)
c       in (3).  Note that the use of fixed order quadrature rules is
c       strongly discouraged.
c       
c       The precomputed angular quadrature rules are stored on the disk,
c       in files named radquads??.txt, where ?? is the order of the 
c       formulae contained in the file.
c
c       The following subroutines are user-callable:
c
c   radinit - store the information necessary to construct quadrature
c       formulae for integrals of the form (1) in a structure rad
c
c   radquad - return a quadrature for the evaluation of integrals of
c       the form (1) over a triangle containing the origin; this 
c       subroutine uses precomputed quadrature rules stored in the
c       "rad" structure to evaluate the outer integral in (3)
c
c   radquad0 - return a quadrature for the evaluation of integrals of
c       the form (1) over a triangle containing the origin; this version
c       of the routine uses piecewise double exponential quadrature rules 
c       to evaluate the outer integral in (3)
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc


        subroutine radinit(ier,norder,rad,lrad,lkeep)
        implicit double precision (a-h,o-z)
        dimension rad(1)
...
c
        ...
        lwhtslege = nlege
c
        ixslege2  = iwhtslege+lwhtslege
        lxslege2  = nlege2
c
        iwhtslege2 = ixslege2+lxslege2
        lwhtslege2 = nlege2
c
        ins = iwhtslege2+lwhtslege2
        lns = nr*nt
...
c        call legequad2(nlege,rad(ixslege2),rad(iwhtslege2))
c
...
c
        ...
        subroutine radquad(ier,rad,verts,maxquad,nquad,xs,ys,whts)
        implicit double precision (a-h,o-z)
...
c   maxquad- the largest quadrature quadrature which can be accomodated
c       by the user-supplied arrays xs, ys and whts
c
c                           Output Parameters:
c
c   ier - an error return code;
...
c   (xs,ys,whts) - the requested quadrature rule
c                   
c      
        ier   = 0
        nquad = 0
...
c
        ...
        maxquad0 = maxquad
c
        call raddiag0(ier,rad,x1,y1,x2,y2,maxquad0,nquad0,xs(nquad+1),
     -    ys(nquad+1),whts(nquad+1),rcut,norder,
     -    nlege,rad(ixslege),rad(iwhtslege))
        if (ier .ne. 0) return
        nquad    = nquad+nquad0
        maxquad0 = maxquad0-nquad0
c
        call raddiag0(ier,rad,x1,y1,x3,y3,maxquad0,nquad0,xs(nquad+1),
     ...
     -    nlege,rad(ixslege),rad(iwhtslege))
        ...
        maxquad0 = maxquad0-nquad0
c
        call raddiag0(ier,rad,x2,y2,x3,y3,maxquad0,nquad0,xs(nquad+1),
     1    ys(nquad+1),whts(nquad+1),rcut,norder,
     -    nlege,rad(ixslege),rad(iwhtslege))
        if (ier .ne. 0) return
        nquad    = nquad+nquad0
...
c
c        n     = 3*(norder+1)+2
c        call raddiag_disk0(ier,n,nlege,rad(ixslege),rad(iwhtslege),rcut,
c     -    maxquad0,nquad0,xs(nquad+1),ys(nquad+1),whts(nquad+1))
c        nquad    = nquad+nquad0
c        maxquad0 = maxquad0-nquad0
c
        end


        subroutine raddiag_disk0(ier,n,nlege,xslege,whtslege,rcut,
     -    maxquad,nquad,xs,ys,whts)
        implicit double precision (a-h,o-z)
        dimension xslege(1),whtslege(1)
        dimension xs(1),ys(1),whts(1)
c
        data pi      / 3.14159265358979323846264338327950288d0 /
c
c       Construct a quadrature for the evaluation of integrals of the form
c
c  
c         \int    f(x,y) da
c            B_r(0)
c
c       where f(x,y) is a function such that f(r\cos(t),r\sin(t)) is of the
c       form
c
c         norder   3*(j+1)+2
c         \sum    \sum r^j { \alpha_j cos(kt) + \beta_j sin(kt) )  
c          j=-2    k=0
c
c       and B_r(0) denotes the ball of radius r centered at 0.  The
c       quadrature is the tensor product of a Legendre rule with a
c       trapezoid rule.
c
c
c                            Input Parameters:
c
c   n - the length of the trapezoid rule to use
c   nlege - the length of the Legendre rule to use
c   (xslege,whtslege) - the nodes and weights of the Legendre rule of
c        order nlege
c   maxquad - the maximum length of the quadrature which can be
c        accomodated by the user-supplied output arrays
c
c
c                           Output Parameters:
c
c   ier - an error return code;
c       ier = 0   indicates successful execution
c       ier = 16  means that the requested quadrature fomrula is
c                 larger than maxquad
c
c   
c   nquad - the length of the requested quadrature formula
c   (xs,ys,whts) - these user-supplied arrays will contain the 
c       requested quadrature rule
c
c
        ier = 0
        nquad = 0 
...
c
        ...
        subroutine raddiag0(ier,rad,x1,y1,x2,y2,maxquad,
     -   nquad,xs,ys,whts,rcut,norder,nlege,xslege,whtslege)
        implicit double precision (a-h,o-z)
        dimension rad(1),xs(1),ys(1),whts(1),verts(2,3)
...
c   ier - an error return code;
c       ier = 0     indicates successful execution
c       ier = 16    means that the requested rule is larger than maxquad
c       ier = 64    means that one of the necessary quadrature rules is 
c                   missing from the table in the rad structure; typically, 
c                   this  indicates that the triangle is very close to 
c                   degenerate
...
c
        call raddiag1(ier,nts,rad(its),rad(itwhts),
     -    maxquad,nquad,xs,ys,whts,alpha,phi,rcut0,norder,
     -    nlege,xslege,whtslege)
c
        do 1000 i=1,nquad
        x = xs(i)
        y = ys(i)
...
c
 ...
 1000 continue
        ...
        subroutine raddiag1(ier,nts,ts,twhts,
     -     maxquad,nquad,xs,ys,whts,alpha,phi,rcut,norder,
     -     nrs,rs,rwhts)
        implicit double precision (a-h,o-z)
        dimension ts(1),twhts(1),amatr(2,2),ainv(2,2)
        dimension xs(1),ys(1),whts(1)
        dimension rs(nrs),rwhts(nrs)
c        dimension rs(100),rwhts(100)
c
        ier = 0
c
        nquad = 0
...
c
c$$$        ier = 1024
c$$$        if (norder .eq. 4)  call pvquad4(ier,r0,r1,nrs,rs,rwhts)
c$$$        if (norder .eq. 8)  call pvquad8(ier,r0,r1,nrs,rs,rwhts)
c$$$        if (norder .eq. 12) call pvquad12(ier,r0,r1,nrs,rs,rwhts)
c$$$        if (norder .eq. 16) call pvquad16(ier,r0,r1,nrs,rs,rwhts)
c$$$c
c$$$        if (ier .ne. 0) return
c
        do 1100 i=1,nrs
c
        r    = (r1-r0)/2*rs(i)+(r1+r0)/2
        rwht = rwhts(i)*(r1-r0)/2
c
        x    = r*cos(t)
        y    = r*sin(t)
        wht  = rwht*twht*r
...
c
        ...
        subroutine radquad0(ier,verts,n,norder,maxquad,nquad,xs,ys,whts)
        implicit double precision (a-h,o-z)
        dimension verts(2,3),xs(1),ys(1),whts(1)
c
c       This subroutine returns a quadrature for the evaluation
c       of integrals of the form (1).  The inner integral is evaluated
c       using precomputed quadratures contained in the pvquad subroutine
c       and piecewise double exponential quadratures which are 
c       constructed on-the-fly.
c
c                            Input Parameters:
c
c   verts - a (2,3) array each column of which gives the coordinates of
c       one vertex of the triangle T
c   n - the order of the piecewise double exponential quadrature to use
c       in the evaluation of the outer integral in (3); or more accurarely,
c       ceiling(n/2) points will be used
c   norder - the order of the quadrature to apply to the inner integral
c       in (3); the available orders are 4, 8, 12 and 16
c   maxquad - the largest quadrature which can be accomodated by the
c       user-supplied arrays xs, ys and whts
c
c                            Output Parameters:
c
c   ier - an error return code;
...
c 
c   nquad - the length of the resulting quadrature formula
c   (xs,ys,whts) - these user-supplied arrays
c
        ier   = 0
        nquad = 0
c
...
c
        maxquad0 = maxquad
        m        = n/2
        call radquad00(ier,norder,m,rcut,x1,y1,x2,y2,
     -   maxquad0,nquad0,xs(nquad+1),ys(nquad+1),whts(nquad+1))
        if (ier .ne. 0) return
        nquad    = nquad+nquad0
        maxquad0 = maxquad0-nquad0
c
        call radquad00(ier,norder,m,rcut,x1,y1,x3,y3,
     -    maxquad0,nquad0,xs(nquad+1),ys(nquad+1),whts(nquad+1))
        if (ier .ne. 0) return
        nquad    = nquad+nquad0
        maxquad0 = maxquad0-nquad0
c
        call radquad00(ier,norder,m,rcut,x2,y2,x3,y3,
     -    maxquad0,nquad0,xs(nquad+1), ys(nquad+1),whts(nquad+1))
        if (ier .ne. 0) return
        nquad    = nquad+nquad0
        maxquad0 = maxquad0-nquad0
c
c$$$        nlege = norder/2+1
c$$$        call radquad01(ier,n,nlege,rcut,
c$$$     -    maxquad0,nquad0,xs(nquad+1),ys(nquad+1),whts(nquad+1))
c$$$        nquad = nquad+nquad0
...
c
        ...
        subroutine radquad00(ier,norder,n,rcut,x1,y1,x2,y2,
     -   maxquad,nquad,xs,ys,whts)
        implicit double precision (a-h,o-z)
        dimension rad(1),xs(1),ys(1),whts(1),verts(2,3)
        dimension amatr(2,2),ainv(2,2)
        dimension rs(100),rwhts(100)
c
        data pi      / 3.14159265358979323846264338327950288d0 /
c
c       Construct a quadrature for functions which admit representation
c       as (2) over a region T / B_rcut(0) where:
c
c       (1) B_rcut(0) is a ball of radius rcut about the origin;
c
c       (2) T is a triangle with vertices (0,0), (x1,y1) and (x2,y2).
c
c                              Input Parameters:
c
c   norder - the order of the precomputed quadrature used to evaluate
c       the inner integral in (3)
c   n - an integer specifying the length of the double exponential
c       quadrature rule used to evaluate 
c   rcut - the radius of the disk to exclude from the origin
c   maxquad - the length of the largest quadrature which can be
c       accomodated by the user-supplied output arrays xs, ys and
c       whts
c
c                              Output Parameters:
...
c       ier = 16    means that the requested quadrature is of length
c                   greater than maxquad
c       ier = 512   means that the triangle is too degenerate to be
c                   accomodated by the precomputed table
c       ier = 1024  means that the value of norder is invalid
c
c   (nquad,xs,ys,whts) - the resulting quadrature rule
c
        nquad = 0
c
        call radtransform(x1,y1,x2,y2,alpha,phi,amatr,ainv,det)
        rcut0 = rcut*sqrt(det)
c
c       Construct the quadrature on the standard triangle.
c
        dd  = 3.9d0
c
c       For extended precision accuracy:
c
c        dd    = 5.9d0
c
        nts   = 0
...
c
        do 1000 j=-n,n
c
        t    = (tanh(pi/2*sinh(j*h))+1)/2
        twht = (h*pi/2*cosh(j*h)*1.0d0/cosh(pi/2*sinh(j*h))**2)/2
        t    = t*phi
        twht = twht*phi
c
        r0   = rcut0
...
c
 ...
 1000 continue
c
c       Map it to the user-specified one.
c
        do 2000 i=1,nquad
        x   = xs(i)
        y   = ys(i)
...
c
 ...
 2000 continue
        ...
        subroutine radquad01(ier,n,nlege,rcut,
     -    maxquad,nquad,xs,ys,whts)
        implicit double precision (a-h,o-z)
        dimension xs(1),ys(1),whts(1)
        dimension xslege(10),whtslege(10)
c
        data pi      / 3.14159265358979323846264338327950288d0 /
c
c       Construct a quadrature for functions of the form (2) over
c       a disk containing the origin.  These quadratures are tensor
c       products of Legendre rules and the trapezoid rule.
c
c                             Input Parameters:
c
c   n - the length of the trapezoid rule to use
c   nlege - the length of the Legendre rule to use
c   rcut - the radius of the disk
c   maxquad - the maximum number of points which can be accomodated
...
c   ier - an error return code;
c       ier = 0   indicates succesful execution
c       ier = 16  means that the requestion quadrature rule is longer
c                 than maxquad
c
c   (nquad,xs,ys,whts) - the resulting quadrature
c 
c
...
c
        ...
        dd = 2*pi/dd
c
        call legequad00(ier,nlege,xslege,whtslege)
c
        r0 = 0
        r1 = rcut
c
...
c
        ...
        data pi /3.14159265358979323846264338327950288d0/
c
c       Use Newton's method and the well-known recurrence relation
c       for Legendre polynomials to compute the nodes and weights of 
c       the n-point Gauss-Legendre quadrature.  The running time of 
c       this procedure grows as O(n**2), but it is quite efficient for 
c       small n.  
c
c                            Input Parameters:
c
...
c
        ...
        subroutine legequad00(ier,n,xs,whts)
        implicit double precision (a-h,o-z)
        dimension xs(n),whts(n)
c
c
c       Return precomputed Legendre quadratures to the user.
c
c                         Input Parameters:
c
c   n - the order of the Legendre quadrature to return; currently
c       allowable values are 3, 5, 7 and 9
c
c                        Output Parameters:
c
c   ier - an error return code;
c       ier = 0     indicates successful execution
c       ier = 1024  means that an invalid value was specified for the
c                   parameter n
c
        dimension xs3(3),whts3(3)
        data xs3 / 
...
c
     ...
     -   0.949107912342758524526189684048D+00/
        data whts7 / 
     -   0.129484966168869693270611432679D+00,
     -   0.279705391489276667901467771424D+00,
     -   0.381830050505118944950369775489D+00,
     -   0.417959183673469387755102040816D+00,
     -   0.381830050505118944950369775489D+00,
     -   0.279705391489276667901467771424D+00,
     -   0.129484966168869693270611432679D+00/
c
        dimension xs9(9),whts9(9)
        data xs9 /
     ...
     -   0.968160239507626089835576202904D+00/
        data whts9 /
     -   0.812743883615744119718921581105D-01,
     -   0.180648160694857404058472031243D+00,
     -   0.260610696402935462318742869419D+00,
     -   0.312347077040002840068630406584D+00,
     -   0.330239355001259763164525069287D+00,
     -   0.312347077040002840068630406584D+00,
     -   0.260610696402935462318742869419D+00,
     -   0.180648160694857404058472031243D+00,
     -   0.812743883615744119718921581105D-01/
