...
c
c$$$cccccccccc
c$$$c       First, test the integration of koornwinder polynomials.
c$$$cccccccccc
c$$$c
...
c
        call raddiag(ier,rad,verts,nquad,xs,ys,whts)
        if (ier .ne. 0) then
...
c
        n      = 1000
        call radquad(ier,verts,n,norder,nquad0,xs0,ys0,whts0)
c
        if (ier .ne. 0) then
...
c
        val = 1 + 1/r
c
c        val = cos(2*t)/r**2 + 1/r +sin(t)**2+r**2
c
        end
...
c$$$c
c$$$        do 1000 k=-n,n
c$$$        t    = (tanh(pi/2*sinh(k*h))+1)/2
c$$$        twht = (h*pi/2*cosh(k*h)*1.0d0/cosh(pi/2*sinh(k*h))**2)/2
c$$$        nts        = nts+1
c$$$        ts(nts)    = t
c$$$        twhts(nts) = twht
c$$$ 1000 continue
