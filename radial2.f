...
c
        ...
        double precision, allocatable :: rad(:),errs(:)
c
...
c
 ...
 0100 continue
c
        call prin2("errs=*",errs,npoly)
        call prin2("errmax = *",errmax,1)
c
c       Test by comparison with a value computed via mathematica.
c
        n = 10 000
        call radquad0(ier,norder,n,verts,
     -    maxquad,nquad0,xs0,ys0,whts0)
        if (ier .ne. 0) then
...
c
c        val = 1
c        val = 1 + 1/r + sin(t)**2*r
c
        val = cos(2*t)/r**2 
c+ 1 + 1/r +sin(t)**2+r**8*cos(12*t)
c
        end
...
c       pvquad (see below) are used to evaluate the inner integral.  The
c       outer integral (in the angular variable) is evaluated using a
c       table of precomputed quadratures stored on the disk.
c 
c       The precomputed angular quadrature rules are stored on the disk,
c       in files named radquads??.txt, where ?? is the order of the 
c       formulae contained in the file.  The quadratures used to 
c       evaluate the integral in r are contained in tables in the
c       pvquad?? subroutines.
c
...
c
c   radquad - return a quadrature for evaluating integrals of the
c       form (1)
c
c   radquad0 - construct a brute-force quadrature formula for
c        evaluating integrals of the form (1); the quadrature formula
c        returned by this routine is constructed using piecewise
c        Legendre rules which are built on-the-fly
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
c                            Input Parameters:
c
c   norder - the order of the quadrature rules used to evaluate the
c       inner integral (in the radial variable r) in (3); at the present
c       time, norder can be 4, 8, 12, 16
c
...
c       ier = 64   means that an invalid value was specified for norderc
c                  or n
c       ier = 128  means that the input file containting the appropriate
c                  precomputed angular quadratures could not be opened
c       ier = 1024 means that a formatting error was encountered while
c                  attempting to read the table of angular quadratures
c                  from the disk
c
c   rad - on return, this user-supplied array will contain the quadrature
c        tables.  The header for the rad structure is as follows:
c
c   rad(1)  -                                                             (nr)
c   rad(2)  - 
c   rad(3)  -  
...
c
        ...
        rad(30) = iwork
c
c       Construct the Legendre quadratures
c
...
c
        ...
        subroutine radquad0(ier,norder,n,verts,
     -    maxquad,nquad,xs,ys,whts)
        implicit double precision (a-h,o-z)
        dimension verts(2,3),xs(1),ys(1),whts(1)
c
        dimension xslege1(3*(norder+1)+2),whtslege1(3*(norder+1)+2)
        dimension xslege2(n),whtslege2(n)

c
c       This subroutine contains a "fixed order" brute force quadrature 
c       for integrating functions of the form (1). 
c
c                             Input Parameters:
c
c
c   norder - order of the quadratrate to be used to evaluate the inner
c       integrals in (1); currently, this value can be 4,8,12 or 16
c   n - the order of the piecewise Legendre rules used to
c        evaluate the outer integrals in (3)
c
c   verts - vertices of the triangle which is the integration domain
c
c   maxquad - the maximum length of the quadrature rule which can be
c       stored in the user-supplied arrays xs,ys and whts
c
c
c   WARNING - this routine allocates O(norder) + O(n) memory from 
c             the stack
c
c                             Output Parameters:
c
...
c   ier - an error return code;
c       ier = 0     indicates successful execution
c       ier = 4     means that the size of the requested quadrature formula
c                   is larger than maxquad
c       ier = 1024  means that an invalid value was specified for norder
c
c   nquad - the length of the resulting quadrature rule
c   (xs,ys,whts) - the resulting quadrature formula
c
        ier         = 0
        nquad       = 0
c
        ...
        n2          = n
c
        call legequad(n1,xslege1,whtslege1)
        call legequad(n2,xslege2,whtslege2)
c
c        if (maxquad .lt. 3*nlege*nlege2+n*nlege) then
c        ier = 4
c        return
c        endif
c
...
c
        call radquad00(norder,nlege2,rad(ixslege2),rad(iwhtslege2),
     -   rcut,x1,y1,x2,y2,
     -   nquad0,xs(nquad+1),ys(nquad+1),whts(nquad+1),rad(iwork))
        nquad = nquad+nquad0
c
        call radquad00(norder,nlege2,rad(ixslege2),rad(iwhtslege2),
     -    rcut,x1,y1,x3,y3,
...
c
        n     = 3*(norder+1)+2
        call raddiag_disk0(nlege1,nlege,rad(ixslege),rad(iwhtslege),
     -    rcut,nquad0,xs(nquad+1),ys(nquad+1),whts(nquad+1))
        nquad = nquad+nquad0
c
        end
        ...
        subroutine radquad00(norder,nts,ts,twhts,
     -   rcut,x1,y1,x2,y2,
     -   nquad,xs,ys,whts,work)
        implicit double precision (a-h,o-z)
        dimension rad(1),xs(1),ys(1),whts(1),verts(2,3)
        dimension ts(1),twhts(1),work(1)
c
...
c
 ...
 1100 continue
        ...
        subroutine raddiag01(nts,ts,twhts,
     1     nquad,xs,ys,whts,alpha,phi,rcut,norder,rs,rwhts)
        implicit double precision (a-h,o-z)
        dimension ts(1),twhts(1),amatr(2,2),ainv(2,2)
        dimension xs(1),ys(1),whts(1)
        dimension rs(10 000),rwhts(10 000)
c
        nquad = 0
        do 1000 j=1,nts
c
...
c
c    maxquad - the length of the largest quadrature formula which
c       can be accomodated by the user-supplied arrays xs,ys and
c       whts
c
c                           Output Parameters:
c
...
c   whts - this user-supplied array will contain the quadrature nodes
c                   
c      
        ier   = 0
        nquad = 0
...
c
        n     = 3*(norder+1)+2
c
        call raddiag_disk0(n,nlege,rad(ixslege),rad(iwhtslege),
     -    rcut,nquad0,xs(nquad+1),ys(nquad+1),whts(nquad+1))
        nquad = nquad+nquad0
c
        end
        ...
        subroutine raddiag_disk0(n,nlege,xslege,whtslege,rcut,
     -    nquad,xs,ys,whts)
        implicit double precision (a-h,o-z)
...
c
c
c       Note: the user is responsible for ensuring that the ouput arrays
...
c
c       with 0 < alpha < 1,  0 < phi < pi.
c
c       Also, return the inverse mapping and the determinant of this
c       mapping.
c
c
c                           Input Parameters:
