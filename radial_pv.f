...
c
     ...
     -    par8)
        ...
        t = atan2(y,x)
c
        val = sin(t)/r**2 + 1/r +sin(7*t)+r**2+cos(7*t)*r**7
c
        end
...
c
        n     = 3*(norder+1)+2
        call raddiag_disk0(ier,n,nlege,rad(ixslege),rad(iwhtslege),rcut,
...
c
 ...
 1000 continue
     ...
     1     maxquad,nquad,xs,ys,whts,alpha,phi,rcut,norder)
        implicit double precision (a-h,o-z)
...
c
        nlege = norder/2+1
        call radquad01(ier,n,nlege,rcut,
     -    maxquad0,nquad0,xs(nquad+1),ys(nquad+1),whts(nquad+1))
        nquad = nquad+nquad0
c
        end
